...
class home {
        ...
        if (req.isAuthenticated()) {
            if (req.user.rolN === 'asistente') {
                ...
                res.render('fragmentos/rolPrincipal', {
                    titulo: 'Telia',
                    regNoticia: 'registro/registrarNoticia',
                    view: 'esRegNoticia',
                    navRol: 'menu/navRol',
                    rol: rol
                });
    ...
    registrarNoticia(req, res) {
        ...
        form.parse(req, function (err, fields, files) {
            console.log("El valor de mult es", fields.multimedia);
    ...
    verNoticia(req, res) {
        ...
        if (req.isAuthenticated()) {
            var rol = req.user.rol;
            sequelize.query("select * , 'true' as ESasistente, DATE(fecha_registro) as DATE from noticia", { type: Sequelize.QueryTypes.SELECT })
                .then(function (nots) {
                    res.render('fragmentos/rolPrincipal',
                        {
                            titulo: "TELIA",
                            lista: nots,
                            view: 'eslistarNoticia',
                            navRol: 'menu/navRol',
                            rol: rol,
                            listarNoticia: 'listar/listarNoticia',
                        });
                }).catch(function (err) {
            ...
            if (req.isAuthenticated()) {
                if (req.user.rolN === 'asistente') {
                    ...
                    res.render('fragmentos/rolPrincipal', {
                        ...
                        editNoticia: 'editar/editarNoticia',
                        view: 'esEditNoticia',
                        navRol: 'menu/navRol',
                        rol: req.user.rol,
                        not: not,
